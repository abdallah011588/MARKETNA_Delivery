...
class AcceptedController extends GetxController
{
  ...
  getData()async{
    ...
    update();
    var response  = await ordersData.getAcceptedData(myServices.shared!.getString("id")!);
  ...
  deliverOrderSuccessfully(String userId , String orderId)async{
    requestStatus = RequestStatus.loading;
    update();
    var response  = await ordersData.deliverOrderSuccessfullyData( userId, orderId);
    requestStatus = handlingData(response);
    if(requestStatus == RequestStatus.success)
    {
      if(response['status']=="success")
      {
        Get.snackbar("Information".tr, "Order".tr+" ${orderId}"+"delivered successfully".tr);
        getData();
        ArchivedController archivedController =Get.put(ArchivedController());
        archivedController.getData();
      }
      else
      {
